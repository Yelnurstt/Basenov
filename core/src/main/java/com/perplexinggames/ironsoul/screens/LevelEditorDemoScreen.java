...
public class LevelEditorDemoScreen implements Screen {
    ...
    public void show() {
        ...
        runtimeLevel = new RuntimeLevel(
            new LevelData("bootstrap", "Bootstrap", 40, 30, 32)
        );
        ...
        levelEditor.applyLoadedWorld(
            initialWorld,
            levelEditor.getLastLoadSourceDescription(),
            levelEditor.wasLastLoadEmptyFallback()
        );

        levelEditor.clearHistory();

        tank = new PhysicsTank(
            runtimeLevel.getTileSize() * 2f,
            getInitialTankSpawnY(runtimeLevel.getTileSize()),
        ...
        transitionService = new WorldTransitionService(
            levelEditor.getWorldData(),
            streamingService,
            new PhysicsTankRuntimeAdapter(tank),
            blockId -> levelEditor.selectActiveBlock(blockId)
        );
    ...
    private void updateGameplay(float delta) {
        ...
        for (Projectile p : projectileSystem.getProjectiles()) {
            ...
            if (!enemy.isDead()
                && p.getBounds().overlaps(enemy.getBounds())) {

                p.destroy();

                projectileSystem.spawnExplosion(
                    p.getX(),
                    p.getY()
                );
                ...
                System.out.println(
                    "Попадание! ХП врага: " + enemy.getHealth()
                );
            }

            else if (p.getY() < tank.physics.y) {
    ...
    private void synchronizeGameplayBlockSelection() {
        ...
        if (!levelEditor.getActiveBlockId().equals(
            streamingService.getCurrentBlock()
        )) {

            streamingService.setCurrentBlock(
                levelEditor.getActiveBlockId()
            );
    ...
    private void spawnTankAtActiveBlock() {
        ...
        if (!activeBlock.spawnPoints.isEmpty()) {
            ...
            transitionService.moveTankToSpawnPoint(
                WorldSpawnResolver.resolveSafeSpawnPoint(
                    activeBlock,
                    activeBlock.spawnPoints.get(0),
                    levelEditor.getWorldData().tileSize
                )
            );
    ...
    private void clampCameraToLevelBounds() {
        ...
        float halfWidth =
            worldCamera.viewportWidth * worldCamera.zoom * 0.5f;

        float halfHeight =
            worldCamera.viewportHeight * worldCamera.zoom * 0.5f;
        ...
        if (worldWidth <= halfWidth * 2f) {
            ...
            worldCamera.position.x = Math.max(
                halfWidth,
                Math.min(
        ...
        if (worldHeight <= halfHeight * 2f) {
            ...
            worldCamera.position.y = Math.max(
                halfHeight,
                Math.min(
    ...
    private void renderOverlay() {
        ...
        String interactionText =
            transitionService == null
                ? null
                : transitionService.getActiveInteractionText();

        if (interactionText != null
            && !interactionText.isBlank()) {

            font.draw(
                batch,
                interactionText,
                hudCamera.viewportWidth * 0.35f,
                48f
            );
        }
    ...
    private String buildOverlayText() {
        ...
        if (levelEditor.getMode() == EditorMode.GAMEPLAY) {
            ...
            return new StringBuilder()
                ...
                .append("BLOCK: ")
                .append(levelEditor.getActiveBlockId())
                .append('\n')
                .append("LOADED: ")
                .append(String.join(", ",
                    streamingService.getLoadedBlocks()))
                .append('\n')
                .append("CONNECTED: ")
                .append(String.join(", ",
                    streamingService.getConnectedBlocks()))
                .append('\n')
                .append("ACTIVE GATE: ")
                .append(formatGate(
                    transitionService.getActiveGate()))
                .append('\n')
                .append("LAST TRANSITION: ")
                .append(transitionService.getLastTransitionLog())
                .append('\n')
        ...
        return new StringBuilder()
            .append("MODE: EDITOR\n")
            .append("BLOCK: ")
            .append(activeBlock == null ? "none" : activeBlock.id)
            .append('\n')
            .append("SOLID: ")
            .append(runtimeLevel.getBlockCount())
            .append(" | TERRAIN: ")
            .append(runtimeLevel.getTerrainPointCount())
            .append(" pts\n")
    ...
    private float getInitialTankSpawnY(float tileSize) {
        ...
        for (TerrainPath terrainPath : runtimeLevel.getTerrainPaths()) {
            ...
            for (com.perplexinggames.ironsoul.terrain.TerrainPoint point
                : terrainPath.getPoints()) {

                highestTerrainY = Math.max(
                    highestTerrainY,
                    point.getY()
                );
            }
        ...
        float maxSpawnY =
            runtimeLevel.getPixelHeight() - tileSize * 2f;

        return MathUtils.clamp(
            highestTerrainY + spawnMargin,
            minSpawnY,
            Math.max(minSpawnY, maxSpawnY)
        );
    }
}
